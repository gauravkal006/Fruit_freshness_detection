...
def generate_webcam_frames(cam_index=0):
    camera = cv2.VideoCapture(cam_index, cv2.CAP_DSHOW) if sys.platform.startswith('win') else cv2.VideoCapture(cam_index)
    if not camera.isOpened():
        for alt_idx in [1, 2, 0]:
            if alt_idx != cam_index:
                camera = cv2.VideoCapture(alt_idx, cv2.CAP_DSHOW) if sys.platform.startswith('win') else cv2.VideoCapture(alt_idx)
                if camera.isOpened():
                    break
